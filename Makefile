...
webpack: $(NODE_MODULES)  ## Runs webpack (useful only for debugging webpack)
	rm -rf dist/static dist/manifest.json
...
clean:  ## Cleans up everything
	rm -rf node_modules .*-updated .*-bin out dist
...
travis-dist: dist  ## Creates a distribution as if we were running on travis
	...
	tar -Jcf out/dist-bin/${TRAVIS_BUILD_NUMBER}.static.tar.xz --transform="s,^dist/static/,," dist/static/*
